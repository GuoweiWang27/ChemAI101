# 教材反应内容生产指南（人教版高中化学）· v1.0（已签收生效）

> **状态：主人已于 2026-08-25 签收（GUI 确认），本指南生效。**
> 首批 40 条经主人授权先行上线（`reviewed: true` 为主人授权口径，见 `docs/specs/reaction-signoff.md` 台账说明）；**任课老师逐条签核为上线后动作**：老师勾 ✔ 的条目保持不变，勾 ✘ 或要求修改的按意见修订或临时下线。
> 机读素材库：`src/data/reactions/_staging/batch-1-draft-v1.json`（与本文档第 8 节清单一一对应；正式库 5 个章节 JSON 与其 id 完全一致）。
...
| 2026-08-25 | v1.0 | **主人 GUI 签收，指南生效**。首批 40 条已按主人授权先行上线（老师签核转上线后动作）；正式库/草稿/审核清单三方 id 核对一致（ox-alpha 落盘） |
