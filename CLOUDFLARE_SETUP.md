...
- 上游：DeepSeek 官方 OpenAI-compatible API，模型 `deepseek-v4-flash`

浏览器只向 Worker 发送业务参数；Worker 在服务端补上
`DEEPSEEK_API_KEY` Secret binding。任何 Vite / Pages 构建变量都不得
保存供应商 key。
...
```bash
npm run build
rg -n "api\\.(deepseek\\.com|vectorengine\\.ai)|Authorization.{0,40}Bearer|sk-[A-Za-z0-9._-]{8,}" dist
```
...
当前正式上游使用 DeepSeek 官方 API。轮换时先创建新 key、更新
`DEEPSEEK_API_KEY` Worker Secret，验证成功后再吊销旧 key。旧的
VectorEngine key 已退出正式链路；若仍可访问原账号，应在原平台撤销。
